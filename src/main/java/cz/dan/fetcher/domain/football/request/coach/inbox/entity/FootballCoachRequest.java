...
@AllArgsConstructor
@SuperBuilder
public class FootballCoachRequest extends Request {
    ...
    @OneToMany(mappedBy = "footballCoachRequest", cascade = ALL, orphanRemoval = true, fetch = EAGER)
    @Default
    private List<FootballCoachRequestFailureDetail> failureDetails = new ArrayList<>();

    @Override
    public void addFailureDetail(String reason) {
        failureDetails.add(FootballCoachRequestFailureDetail.builder()
                .footballCoachRequest(this)
                .reason(reason)
                .build());
    }
