...
class FootballPlayerOutboxJobTest {
    ...
    @BeforeEach
    void setUp() {
        sut = new FootballPlayerOutboxJob(jobProperties, footballPlayerRequestOutboxService,
                Set.of(footballPlayerRequestOutboxSender));
    }

    @Test
    void sendsOutboxAndDeletesIt() {
        when(footballPlayerRequestOutboxService.getAll())
        ...
        verify(footballPlayerRequestOutboxService, times(1)).delete(1L);
    }
    ...
    void doesNotDeleteOutboxIfSendingFails() {
        ...
        verifyNoMoreInteractions(footballPlayerRequestOutboxService);
    }
